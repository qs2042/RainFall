    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <template v-else-if="column.dataIndex === 'enabled'">
          <a-switch @change="table.onUpdate(record)" v-model:checked="record.enabled"></a-switch>
        </template>

        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','name','cronExpression','invokeTarget','cronDescribe'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <a-modal title="新增task数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="表达式">
          <a-input class="mb-1" v-model:value="form.cronExpression" placeholder="请输入: 表达式"></a-input>
        </a-form-item>
        <a-form-item label="调用目标">
          <a-input class="mb-1" v-model:value="form.invokeTarget" placeholder="请输入: 调用目标"></a-input>
        </a-form-item>
        <a-form-item label="描述">
          <a-input class="mb-1" v-model:value="form.cronDescribe" placeholder="请输入: 描述"></a-input>
        </a-form-item>
        <a-form-item label="是否启用">
          <a-switch class="mb-1" v-model:checked="form.enabled" checked-children="开" un-checked-children="关"></a-switch>
        </a-form-item>
...
// api
import { ITask, taskApi } from "@/api/erp_system/Task.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    taskApi.queryPage(page, table.show).then(({data}) => {
...
const table = reactive({
  ...
  columns: [
    ...
    {
      title: '表达式', dataIndex: 'cronExpression', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.cronExpression.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '调用目标', dataIndex: 'invokeTarget', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.invokeTarget.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '描述', dataIndex: 'cronDescribe', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.cronDescribe.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    { title: '是否启用', dataIndex: 'enabled', key: 'id'},
    {
