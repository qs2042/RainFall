<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tMessageBoard数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="用户主键">
                <a-input class="mb-1" v-model:value="form.user_id" placeholder="请输入: 用户主键"></a-input>
        </a-form-item>
        <a-form-item label="留言内容">
                <a-input class="mb-1" v-model:value="form.content" placeholder="请输入: 留言内容"></a-input>
        </a-form-item>
...
// api
import { ITMessageBoard, tMessageBoardApi } from "@/api/bbs/TMessageBoard.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tMessageBoardApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      tMessageBoardApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '用户主键', dataIndex: 'userId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '留言内容', dataIndex: 'content', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 删除
  onDelete: (id) => {
      tMessageBoardApi.remove(id).then(() => {
          message.success("删除成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("删除失败")
  ...
  // 更新
  onUpdate: (item) => {
      tMessageBoardApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      tMessageBoardApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<ITMessageBoard>({
  id: "",
  user_id: "",
  content: "",
  create_time: "",
...
const modal = reactive({
  ...
  handleOk: (e: MouseEvent) => {
    modal.visible = false
    tMessageBoardApi.add(form).then(()=> {
