  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="[].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增tUser数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="">
                <a-input class="mb-1" v-model:value="form.id" placeholder="请输入: "></a-input>
        </a-form-item>
        <a-form-item label="UUID">
                <a-input class="mb-1" v-model:value="form.uuid" placeholder="请输入: UUID"></a-input>
        </a-form-item>
        <a-form-item label="账号">
                <a-input class="mb-1" v-model:value="form.username" placeholder="请输入: 账号"></a-input>
        </a-form-item>
        <a-form-item label="密码">
                <a-input class="mb-1" v-model:value="form.password" placeholder="请输入: 密码"></a-input>
        </a-form-item>
        <a-form-item label="年龄">
                <a-input class="mb-1" v-model:value="form.age" placeholder="请输入: 年龄"></a-input>
        </a-form-item>
        <a-form-item label="性别">
                <a-input class="mb-1" v-model:value="form.sex" placeholder="请输入: 性别"></a-input>
        </a-form-item>
        <a-form-item label="QQ">
                <a-input class="mb-1" v-model:value="form.qq" placeholder="请输入: QQ"></a-input>
        </a-form-item>
        <a-form-item label="手机号">
                <a-input class="mb-1" v-model:value="form.phone" placeholder="请输入: 手机号"></a-input>
        </a-form-item>
        <a-form-item label="邮箱号">
                <a-input class="mb-1" v-model:value="form.email" placeholder="请输入: 邮箱号"></a-input>
        </a-form-item>
        <a-form-item label="个人介绍">
                <a-input class="mb-1" v-model:value="form.introduce" placeholder="请输入: 个人介绍"></a-input>
        </a-form-item>
        <a-form-item label="头像">
                <a-input class="mb-1" v-model:value="form.image" placeholder="请输入: 头像"></a-input>
        </a-form-item>
        <a-form-item label="注册时间">
                <a-input class="mb-1" v-model:value="form.create_time" placeholder="请输入: 注册时间"></a-input>
        </a-form-item>
      </a-form>
    </a-modal>
  </main>
...
// api
import { ITUser, tUserApi } from "@/api/bbs/TUser.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tUserApi.queryPage(page, table.show).then(({data}) => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: 'QQ', dataIndex: 'qq', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.Qq.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '邮箱号', dataIndex: 'email', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '个人介绍', dataIndex: 'introduce', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.Introduce.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '头像', dataIndex: 'image', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.Image.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
