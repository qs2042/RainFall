<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tJurisdiction数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="权限类型">
                <a-input class="mb-1" v-model:value="form.type" placeholder="请输入: 权限类型"></a-input>
        </a-form-item>
      </a-form>
    </a-modal>
  </main>
...
// api
import { ITJurisdiction, tJurisdictionApi } from "@/api/bbs/TJurisdiction.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tJurisdictionApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      tJurisdictionApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '权限类型', dataIndex: 'type', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 删除
  onDelete: (id) => {
      tJurisdictionApi.remove(id).then(() => {
          message.success("删除成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("删除失败")
  ...
  // 更新
  onUpdate: (item) => {
      tJurisdictionApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      tJurisdictionApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<ITJurisdiction>({
  id: "",
  user_id: "",
  type: "",
})
...
const modal = reactive({
  ...
  handleOk: (e: MouseEvent) => {
    modal.visible = false
    tJurisdictionApi.add(form).then(()=> {
