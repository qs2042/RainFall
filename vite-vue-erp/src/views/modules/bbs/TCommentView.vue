<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tComment数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="帖子主键ID">
                <a-input class="mb-1" v-model:value="form.posts_id" placeholder="请输入: 帖子主键ID"></a-input>
        </a-form-item>
        <a-form-item label="用户主键ID">
                <a-input class="mb-1" v-model:value="form.user_id" placeholder="请输入: 用户主键ID"></a-input>
        </a-form-item>
        <a-form-item label="评论主键(可选)">
                <a-input class="mb-1" v-model:value="form.parent_id" placeholder="请输入: 评论主键(可选)"></a-input>
        </a-form-item>
        <a-form-item label="评论内容">
                <a-input class="mb-1" v-model:value="form.content" placeholder="请输入: 评论内容"></a-input>
        </a-form-item>
...
// api
import { ITComment, tCommentApi } from "@/api/bbs/TComment.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tCommentApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      tCommentApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '帖子主键ID', dataIndex: 'postsId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.PostsId.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '评论主键(可选)', dataIndex: 'parentId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '评论内容', dataIndex: 'content', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
