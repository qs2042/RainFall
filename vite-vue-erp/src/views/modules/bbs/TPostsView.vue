<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tPosts数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="分区主键ID">
                <a-input class="mb-1" v-model:value="form.partition_id" placeholder="请输入: 分区主键ID"></a-input>
        </a-form-item>
        ...
        <a-form-item label="帖子图标">
                <a-input class="mb-1" v-model:value="form.icon" placeholder="请输入: 帖子图标"></a-input>
        </a-form-item>
...
// api
import { ITPosts, tPostsApi } from "@/api/bbs/TPosts.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tPostsApi.queryPage(page, table.show).then(({data}) => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '分区主键ID', dataIndex: 'partitionId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.PartitionId.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '帖子图标', dataIndex: 'icon', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
