<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tModule数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="模块标题">
                <a-input class="mb-1" v-model:value="form.title" placeholder="请输入: 模块标题"></a-input>
        </a-form-item>
        <a-form-item label="模块图标">
                <a-input class="mb-1" v-model:value="form.icon" placeholder="请输入: 模块图标"></a-input>
        </a-form-item>
        <a-form-item label="模块介绍">
                <a-input class="mb-1" v-model:value="form.content" placeholder="请输入: 模块介绍"></a-input>
        </a-form-item>
...
// api
import { ITModule, tModuleApi } from "@/api/bbs/TModule.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tModuleApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      tModuleApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '模块标题', dataIndex: 'title', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '模块图标', dataIndex: 'icon', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '模块介绍', dataIndex: 'content', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
