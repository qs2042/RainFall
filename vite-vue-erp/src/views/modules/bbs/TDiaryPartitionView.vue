<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tDiaryPartition数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="分区图片">
                <a-input class="mb-1" v-model:value="form.image" placeholder="请输入: 分区图片"></a-input>
        </a-form-item>
...
// api
import { ITDiaryPartition, tDiaryPartitionApi } from "@/api/bbs/TDiaryPartition.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tDiaryPartitionApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      tDiaryPartitionApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '分区图片', dataIndex: 'image', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 删除
  onDelete: (id) => {
      tDiaryPartitionApi.remove(id).then(() => {
          message.success("删除成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("删除失败")
  ...
  // 更新
  onUpdate: (item) => {
      tDiaryPartitionApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      tDiaryPartitionApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<ITDiaryPartition>({
  id: "",
  title: "",
  image: "",
  content: "",
...
const modal = reactive({
  ...
  handleOk: (e: MouseEvent) => {
    modal.visible = false
    tDiaryPartitionApi.add(form).then(()=> {
