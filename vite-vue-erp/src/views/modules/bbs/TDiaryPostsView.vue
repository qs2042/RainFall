<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tDiaryPosts数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="分区主键ID">
                <a-input class="mb-1" v-model:value="form.diary_partition_id" placeholder="请输入: 分区主键ID"></a-input>
        </a-form-item>
        <a-form-item label="用户主键ID">
                <a-input class="mb-1" v-model:value="form.user_id" placeholder="请输入: 用户主键ID"></a-input>
        </a-form-item>
        <a-form-item label="帖子标题">
                <a-input class="mb-1" v-model:value="form.title" placeholder="请输入: 帖子标题"></a-input>
        </a-form-item>
        <a-form-item label="帖子图片">
                <a-input class="mb-1" v-model:value="form.image" placeholder="请输入: 帖子图片"></a-input>
        </a-form-item>
        <a-form-item label="帖子内容">
                <a-input class="mb-1" v-model:value="form.content" placeholder="请输入: 帖子内容"></a-input>
        </a-form-item>
...
// api
import { ITDiaryPosts, tDiaryPostsApi } from "@/api/bbs/TDiaryPosts.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tDiaryPostsApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      tDiaryPostsApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '分区主键ID', dataIndex: 'diaryPartitionId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.DiaryPartitionId.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '用户主键ID', dataIndex: 'userId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '帖子标题', dataIndex: 'title', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '帖子图片', dataIndex: 'image', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '帖子内容', dataIndex: 'content', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 更新
  onUpdate: (item) => {
      tDiaryPostsApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      tDiaryPostsApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<ITDiaryPosts>({
  id: "",
  diary_partition_id: "",
  user_id: "",
  title: "",
