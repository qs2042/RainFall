<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tPartition数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="模块主键ID">
                <a-input class="mb-1" v-model:value="form.module_id" placeholder="请输入: 模块主键ID"></a-input>
        </a-form-item>
        <a-form-item label="分区标题">
                <a-input class="mb-1" v-model:value="form.title" placeholder="请输入: 分区标题"></a-input>
        </a-form-item>
        <a-form-item label="分区图标">
                <a-input class="mb-1" v-model:value="form.icon" placeholder="请输入: 分区图标"></a-input>
        </a-form-item>
        <a-form-item label="分区介绍">
                <a-input class="mb-1" v-model:value="form.content" placeholder="请输入: 分区介绍"></a-input>
        </a-form-item>
...
// api
import { ITPartition, tPartitionApi } from "@/api/bbs/TPartition.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tPartitionApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      tPartitionApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '模块主键ID', dataIndex: 'moduleId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.ModuleId.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '分区标题', dataIndex: 'title', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '分区图标', dataIndex: 'icon', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '分区介绍', dataIndex: 'content', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 更新
  onUpdate: (item) => {
      tPartitionApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      tPartitionApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<ITPartition>({
  id: "",
  module_id: "",
  title: "",
  icon: "",
