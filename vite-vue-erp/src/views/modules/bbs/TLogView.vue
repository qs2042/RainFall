<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增tLog数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="用户ID">
                <a-input class="mb-1" v-model:value="form.user_id" placeholder="请输入: 用户ID"></a-input>
        </a-form-item>
        <a-form-item label="模块名">
                <a-input class="mb-1" v-model:value="form.module" placeholder="请输入: 模块名"></a-input>
        </a-form-item>
        <a-form-item label="方法名">
                <a-input class="mb-1" v-model:value="form.method" placeholder="请输入: 方法名"></a-input>
        </a-form-item>
        <a-form-item label="日志类型">
                <a-input class="mb-1" v-model:value="form.type" placeholder="请输入: 日志类型"></a-input>
        </a-form-item>
        <a-form-item label="日志内容">
                <a-input class="mb-1" v-model:value="form.content" placeholder="请输入: 日志内容"></a-input>
        </a-form-item>
        ...
        <a-form-item label="来源URL">
                <a-input class="mb-1" v-model:value="form.url" placeholder="请输入: 来源URL"></a-input>
        </a-form-item>
        <a-form-item label="来源IP">
                <a-input class="mb-1" v-model:value="form.ip" placeholder="请输入: 来源IP"></a-input>
        </a-form-item>
        <a-form-item label="来源MAC">
                <a-input class="mb-1" v-model:value="form.mac" placeholder="请输入: 来源MAC"></a-input>
        </a-form-item>
        <a-form-item label="创建时间">
                <a-input class="mb-1" v-model:value="form.create_time" placeholder="请输入: 创建时间"></a-input>
        </a-form-item>
      </a-form>
    </a-modal>
  </main>
...
// api
import { ITLog, tLogApi } from "@/api/bbs/TLog.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    tLogApi.queryPage(page, table.show).then(({data}) => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '模块名', dataIndex: 'module', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '方法名', dataIndex: 'method', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '日志类型', dataIndex: 'type', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '日志内容', dataIndex: 'content', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '来源URL', dataIndex: 'url', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '来源IP', dataIndex: 'ip', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '来源MAC', dataIndex: 'mac', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.Mac.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
