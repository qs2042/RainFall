  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','name','url','logoUrl','description','sort'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增friendLink数据" v-model:visible="modal.visible" @ok="modal.handleOk">
...
<!--        <a-form-item label="ID">
        ...
        <a-form-item label="名称">
          <a-input class="mb-1" v-model:value="form.name" placeholder="请输入: 名称"></a-input>
        </a-form-item>
        <a-form-item label="URL">
          <a-input class="mb-1" v-model:value="form.url" placeholder="请输入: URL"></a-input>
        </a-form-item>
        <a-form-item label="Logo URL">
          <a-input class="mb-1" v-model:value="form.logoUrl" placeholder="请输入: Logo URL"></a-input>
        </a-form-item>
...
<!--        <a-form-item label="创建时间">
        ...
        <a-form-item label="更新时间">
          <a-input class="mb-1" v-model:value="form.updateTime" placeholder="请输入: 更新时间"></a-input>
        </a-form-item>-->
      </a-form>
    </a-modal>
  </main>
...
// api
import { IFriendLink, friendLinkApi } from "@/api/erp_system/FriendLink.ts";
import {resource,iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    friendLinkApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      friendLinkApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
    ...
    {
      title: 'Logo URL', dataIndex: 'logoUrl', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.logoUrl.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
  ...
  // 更新
  onUpdate: (item) => {
    friendLinkApi.update(item).then(() => {
  ...
  save: (key: string) => {
    ...
    friendLinkApi.update(arr[0]).then(() => {
      message.success("修改成功")
      a.queryPage(table.pagination.current)
    }).catch(() => {
      message.error("修改失败")
...
// 表单
const form = reactive<IFriendLink>({
  id: null as number,
  name: "",
  url: "",
  logoUrl: "",
