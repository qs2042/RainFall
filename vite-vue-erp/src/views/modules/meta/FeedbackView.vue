  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','userId','text','rating','category','status'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
          ...
          <template v-else-if="column.dataIndex === 'rating'">
            <a-rate disabled :value="record.rating"></a-rate>
          </template>
          <template v-else-if="column.dataIndex === 'category'">
            <a-select :value="text" style="min-width: 120px" >
              <a-select-option :value="1">正常反馈</a-select-option>
              <a-select-option :value="2">BUG报告</a-select-option>
              <a-select-option :value="3">功能请求</a-select-option>
              <a-select-option :value="4">危险漏洞</a-select-option>
            </a-select>
          </template>
    ...
    <!-- modal -->
    <a-modal title="新增feedback数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="反馈文本">
          <a-input class="mb-1" v-model:value="form.text" placeholder="请输入: 反馈文本"></a-input>
        </a-form-item>
        <a-form-item label="反馈提交日期">
          <a-input class="mb-1" v-model:value="form.date" placeholder="请输入: 反馈提交日期"></a-input>
        </a-form-item>
        <a-form-item label="反馈评分">
          <a-input-number class="mb-1 w-100" v-model:value="form.rating" placeholder="请输入: 反馈评分"></a-input-number>
        </a-form-item>
        <a-form-item label="反馈分类">
          <a-input-number class="mb-1 w-100" v-model:value="form.category" placeholder="请输入: 反馈分类"></a-input-number>
        </a-form-item>
        <a-form-item label="反馈处理状态">
          <a-input-number class="mb-1 w-100" v-model:value="form.status" placeholder="请输入: 反馈处理状态"></a-input-number>
        </a-form-item>
...
// api
import { IFeedback, feedbackApi } from "@/api/erp_system/Feedback.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    feedbackApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      feedbackApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
    ...
    {
      title: '反馈文本', dataIndex: 'text', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
    ...
    {
      title: '反馈提交日期', dataIndex: 'date', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
    ...
    {
      title: '反馈评分', dataIndex: 'rating', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.rating.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '反馈分类', dataIndex: 'category', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
    ...
    {
      title: '反馈处理状态', dataIndex: 'status', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
