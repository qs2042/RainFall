  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','title','imageUrl','linkUrl'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
          ...
          <template v-else-if="column.dataIndex === 'linkUrl'">
            <a-button v-if="text.length === 0" size="small" danger disabled>跳转</a-button>
            <a-button v-else size="small" target="_blank" :href="text">跳转</a-button>
          </template>
          <!-- 如果是图片 -->
          <template v-else-if="column.dataIndex === 'imageUrl'">
            <a-image class="object-fit-cover" :fallback="resource.getPublicFile('vite.svg')" :width="32" :height="32" :src="text" />
          </template>
        </template>
    ...
    <!-- modal -->
    <a-modal title="新增advertisements数据" v-model:visible="modal.visible" @ok="modal.handleOk">
...
<!--        <a-form-item label="广告唯一标识符">
          <a-input-number class="mb-1 w-100" v-model:value="form.id" placeholder="请输入: 广告唯一标识符"></a-input-number>
        </a-form-item>-->
        <a-form-item label="标题">
          <a-input class="mb-1" v-model:value="form.title" placeholder="请输入: 标题"></a-input>
        </a-form-item>
        <a-form-item label="图片 URL 地址">
          <a-input class="mb-1" v-model:value="form.imageUrl" placeholder="请输入: 图片 URL 地址"></a-input>
        </a-form-item>
        <a-form-item label="链接 URL 地址">
          <a-input class="mb-1" v-model:value="form.linkUrl" placeholder="请输入: 链接 URL 地址"></a-input>
        </a-form-item>
        <a-form-item label="展示开始日期">
          <a-input class="mb-1" v-model:value="form.startDate" placeholder="请输入: 展示开始日期"></a-input>
        </a-form-item>
        <a-form-item label="展示结束日期">
          <a-input class="mb-1" v-model:value="form.endDate" placeholder="请输入: 展示结束日期"></a-input>
        </a-form-item>
...
// api
import { IAdvertisements, advertisementsApi } from "@/api/erp_system/Advertisements.ts";
import {resource,iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    advertisementsApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      advertisementsApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
    {
      title: '广告唯一标识符', dataIndex: 'id', key: 'id',
    ...
    {
      title: '图片 URL 地址', dataIndex: 'imageUrl', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.imageUrl.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '链接 URL 地址', dataIndex: 'linkUrl', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.linkUrl.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '展示开始日期', dataIndex: 'startDate', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.startDate.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '展示结束日期', dataIndex: 'endDate', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
  ...
  // 删除
  onDelete: (id) => {
    advertisementsApi.remove(id).then(() => {
      message.success("删除成功")
      a.queryPage(table.pagination.current)
    }).catch(() => {
      message.error("删除失败")
  ...
  // 更新
  onUpdate: (item) => {
    advertisementsApi.update(item).then(() => {
  ...
  save: (key: string) => {
    ...
    advertisementsApi.update(arr[0]).then(() => {
      message.success("修改成功")
      a.queryPage(table.pagination.current)
    }).catch(() => {
      message.error("修改失败")
...
// 表单
const form = reactive<IAdvertisements>({
  id: null as number,
  title: "",
  imageUrl: "",
  linkUrl: "",
...
const modal = reactive({
  ...
  handleOk: (e: MouseEvent) => {
    modal.visible = false
    advertisementsApi.add(form).then(()=> {
