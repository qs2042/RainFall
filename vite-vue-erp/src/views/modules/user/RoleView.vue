  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','code','name','label','introduce'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增role数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="代码(唯一)">
                <a-input class="mb-1" v-model:value="form.code" placeholder="请输入: 代码(唯一)"></a-input>
        </a-form-item>
        <a-form-item label="名称">
                <a-input class="mb-1" v-model:value="form.name" placeholder="请输入: 名称"></a-input>
        </a-form-item>
        <a-form-item label="标签">
                <a-input class="mb-1" v-model:value="form.label" placeholder="请输入: 标签"></a-input>
        </a-form-item>
        <a-form-item label="介绍">
                <a-input class="mb-1" v-model:value="form.introduce" placeholder="请输入: 介绍"></a-input>
        </a-form-item>
...
// api
import { IRole, roleApi } from "@/api/erp_member/Role.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    roleApi.queryPage(page, table.show).then(({data}) => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '代码(唯一)', dataIndex: 'code', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '标签', dataIndex: 'label', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '介绍', dataIndex: 'introduce', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
