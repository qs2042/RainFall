  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','uuid','name','info','notes','parentId'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增organization数据" v-model:visible="modal.visible" @ok="modal.handleOk">
...
<!--        <a-form-item label="ID">
        ...
        <a-form-item label="上级机构">
                <a-input-number class="mb-1 w-100" v-model:value="form.parentId" placeholder="请输入: 上级机构"></a-input-number>
        </a-form-item>
      </a-form>
    </a-modal>
  </main>
...
// api
import { IOrganization, organizationApi } from "@/api/erp_member/Organization.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    organizationApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      organizationApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '上级机构', dataIndex: 'parentId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 更新
  onUpdate: (item) => {
      organizationApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      organizationApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<IOrganization>({
  id: null as number,
  uuid: "",
  name: "",
  info: "",
