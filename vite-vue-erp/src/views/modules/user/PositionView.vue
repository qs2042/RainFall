  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','departmentId','name','info','notes'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增position数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="部门ID">
                <a-input-number class="mb-1 w-100" v-model:value="form.departmentId" placeholder="请输入: 部门ID"></a-input-number>
        </a-form-item>
...
// api
import { IPosition, positionApi } from "@/api/erp_member/Position.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    positionApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      positionApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '部门ID', dataIndex: 'departmentId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.departmentId.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
...
// 表单
const form = reactive<IPosition>({
  id: null as number,
  departmentId: null as number,
  name: "",
  info: "",
  notes: "",
})
