  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','organizationId','name','info','notes'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增department数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="机构ID">
                <a-input-number class="mb-1 w-100" v-model:value="form.organizationId" placeholder="请输入: 机构ID"></a-input-number>
        </a-form-item>
        <a-form-item label="名称">
                <a-input class="mb-1" v-model:value="form.name" placeholder="请输入: 名称"></a-input>
        </a-form-item>
        <a-form-item label="信息">
                <a-input class="mb-1" v-model:value="form.info" placeholder="请输入: 信息"></a-input>
        </a-form-item>
...
// api
import { IDepartment, departmentApi } from "@/api/erp_member/Department.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    departmentApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      departmentApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '机构ID', dataIndex: 'organizationId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.organizationId.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '信息', dataIndex: 'info', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 更新
  onUpdate: (item) => {
      departmentApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      departmentApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<IDepartment>({
  id: null as number,
  organizationId: null as number,
  name: "",
  info: "",
  notes: "",
})
