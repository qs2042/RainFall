  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','userId','customCurrency'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增userWallet数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="纸币">
                <a-input class="mb-1" v-model:value="form.coins" placeholder="请输入: 纸币"></a-input>
        </a-form-item>
        <a-form-item label="金币">
                <a-input class="mb-1" v-model:value="form.gold" placeholder="请输入: 金币"></a-input>
        </a-form-item>
        <a-form-item label="钻石">
                <a-input class="mb-1" v-model:value="form.diamonds" placeholder="请输入: 钻石"></a-input>
        </a-form-item>
        <a-form-item label="自定义货币({id, balance})">
                <a-input class="mb-1" v-model:value="form.customCurrency" placeholder="请输入: 自定义货币({id, balance})"></a-input>
        </a-form-item>
...
// api
import { IUserWallet, userWalletApi } from "@/api/erp_member/UserWallet.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    userWalletApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      userWalletApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '纸币', dataIndex: 'coins', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.coins.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '金币', dataIndex: 'gold', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.gold.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '钻石', dataIndex: 'diamonds', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.diamonds.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '自定义货币({id, balance})', dataIndex: 'customCurrency', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.customCurrency.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
  ...
  // 更新
  onUpdate: (item) => {
      userWalletApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      userWalletApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<IUserWallet>({
  id: null as number,
