  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','userId','itemId','name','description','imageUrl','nbt'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增userInventorUnique数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="物品ID">
                <a-input-number class="mb-1 w-100" v-model:value="form.itemId" placeholder="请输入: 物品ID"></a-input-number>
        </a-form-item>
        <a-form-item label="自定义名称">
                <a-input class="mb-1" v-model:value="form.name" placeholder="请输入: 自定义名称"></a-input>
        </a-form-item>
        <a-form-item label="自定义描述">
                <a-input class="mb-1" v-model:value="form.description" placeholder="请输入: 自定义描述"></a-input>
        </a-form-item>
        <a-form-item label="自定义图片URL">
                <a-input class="mb-1" v-model:value="form.imageUrl" placeholder="请输入: 自定义图片URL"></a-input>
        </a-form-item>
        <a-form-item label="Named Binary Tag">
                <a-input class="mb-1" v-model:value="form.nbt" placeholder="请输入: Named Binary Tag"></a-input>
        </a-form-item>
...
// api
import { IUserInventorUnique, userInventorUniqueApi } from "@/api/erp_member/UserInventorUnique.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    userInventorUniqueApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      userInventorUniqueApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      {
          title: '物品ID', dataIndex: 'itemId', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.itemId.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
      ...
      {
          title: '自定义名称', dataIndex: 'name', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '自定义描述', dataIndex: 'description', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '自定义图片URL', dataIndex: 'imageUrl', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: 'Named Binary Tag', dataIndex: 'nbt', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.nbt.toString().toLowerCase().includes(value.toLowerCase()),
          onFilterDropdownVisibleChange: visible => {
              if (visible) {
                  setTimeout(() => {
                      tableFilterSearchInput.value.focus();
  ...
  // 删除
  onDelete: (id) => {
      userInventorUniqueApi.remove(id).then(() => {
          message.success("删除成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("删除失败")
  ...
  // 更新
  onUpdate: (item) => {
      userInventorUniqueApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      userInventorUniqueApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<IUserInventorUnique>({
  id: null as number,
...
const modal = reactive({
  ...
  handleOk: (e: MouseEvent) => {
    modal.visible = false
    userInventorUniqueApi.add(form).then(()=> {
