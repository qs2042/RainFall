  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','userId','itemId','quantity'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增userInventory数据" v-model:visible="modal.visible" @ok="modal.handleOk">
      <a-form layout="horizontal" v-bind="{ labelCol: { span: 4 }, wrapperCol: { span: 14 } }"
        ...
        <a-form-item label="唯一物品的唯一标识符">
                <a-input-number class="mb-1 w-100" v-model:value="form.id" placeholder="请输入: 唯一物品的唯一标识符"></a-input-number>
        </a-form-item>
        ...
        <a-form-item label="物品数量">
                <a-input-number class="mb-1 w-100" v-model:value="form.quantity" placeholder="请输入: 物品数量"></a-input-number>
        </a-form-item>
        <a-form-item label="获取时间">
                <a-input class="mb-1" v-model:value="form.createdAt" placeholder="请输入: 获取时间"></a-input>
        </a-form-item>
...
// vue
import {ref, reactive, onMounted, defineProps, toRefs, onBeforeMount, watch, onUpdated, onBeforeUpdate} from "vue";
...
// api
import { IUserInventory, userInventoryApi } from "@/api/erp_member/UserInventory.ts";
...
const a = {
  queryPage: (page) => {
    userInventoryApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      userInventoryApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      {
          title: '唯一物品的唯一标识符', dataIndex: 'id', key: 'id',
      ...
      {
          title: '物品数量', dataIndex: 'quantity', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
      ...
      {
          title: '获取时间', dataIndex: 'createdAt', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
  ...
  // 删除
  onDelete: (id) => {
      userInventoryApi.remove(id).then(() => {
          message.success("删除成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("删除失败")
  ...
  // 更新
  onUpdate: (item) => {
      userInventoryApi.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      userInventoryApi.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 挂载DOM后加载数据
onMounted(() => {
  table.datasource.push(...data.value)
})
// 数据更新还未渲染时加载数据
onBeforeUpdate(() => {
  // 如果不是请求的数据, 就清空之前的数据
  if (!getFlagAndSetFalse()) {
    table.datasource = []
  }
  // 重新赋值
  table.datasource.push(...data.value)
...
// 表单
const form = reactive<IUserInventory>({
  id: null as number,
...
const modal = reactive({
  ...
  handleOk: (e: MouseEvent) => {
    modal.visible = false
    userInventoryApi.add(form).then(()=> {
