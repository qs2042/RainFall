  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','key','value','description'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增params数据" v-model:visible="modal.visible" @ok="modal.handleOk">
...
<!--        <a-form-item label="ID">
        ...
        <a-form-item label="键">
          <a-input class="mb-1" v-model:value="form.key" placeholder="请输入: 键"></a-input>
        </a-form-item>
        <a-form-item label="值">
...
// api
import { IParams, paramsApi } from "@/api/erp_system/Params.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    paramsApi.queryPage(page, table.show).then(({data}) => {
