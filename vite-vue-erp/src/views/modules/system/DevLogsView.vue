  ...
  <main>
    ...
    <a-table row-key="id" :size="table.size"
      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <!-- 如果是需要被编辑的字段(str, int) -->
        <template v-else-if="['id','userId','projectId','taskName','taskDescription'].includes(column.dataIndex)">
          <!-- 如果正在编辑字段中 -->
    ...
    <!-- modal -->
    <a-modal title="新增devLogs数据" v-model:visible="modal.visible" @ok="modal.handleOk">
...
<!--        <a-form-item label="日志ID">
          <a-input-number class="mb-1 w-100" v-model:value="form.id" placeholder="请输入: 日志ID"></a-input-number>
        </a-form-item>-->
        <a-form-item label="用户ID">
          <a-input-number class="mb-1 w-100" v-model:value="form.userId" placeholder="请输入: 用户ID"></a-input-number>
        </a-form-item>
        <a-form-item label="项目ID">
          <a-input-number class="mb-1 w-100" v-model:value="form.projectId" placeholder="请输入: 项目ID"></a-input-number>
        </a-form-item>
        <a-form-item label="任务名称">
          <a-input class="mb-1" v-model:value="form.taskName" placeholder="请输入: 任务名称"></a-input>
        </a-form-item>
        <a-form-item label="任务描述">
          <a-input class="mb-1" v-model:value="form.taskDescription" placeholder="请输入: 任务描述"></a-input>
        </a-form-item>
        <a-form-item label="花费的小时数">
          <a-input class="mb-1" v-model:value="form.hoursSpent" placeholder="请输入: 花费的小时数"></a-input>
        </a-form-item>
...
// api
import { IDevLogs, devLogsApi } from "@/api/erp_system/DevLogs.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    devLogsApi.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      devLogsApi.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
    ...
    {
      title: '项目ID', dataIndex: 'projectId', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.projectId.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '任务名称', dataIndex: 'taskName', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
    ...
    {
      title: '任务描述', dataIndex: 'taskDescription', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.taskDescription.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
    ...
    {
      title: '花费的小时数', dataIndex: 'hoursSpent', key: 'id',
      customFilterDropdown: true,
      onFilter: (value, record) =>
          record.hoursSpent.toString().toLowerCase().includes(value.toLowerCase()),
      onFilterDropdownVisibleChange: visible => {
        if (visible) {
          setTimeout(() => {
            tableFilterSearchInput.value.focus();
