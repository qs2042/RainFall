<template>
  <main>
    ...
    <!-- modal -->
    <a-modal title="新增${tableNameCamelCase}数据" v-model:visible="modal.visible" @ok="modal.handleOk">
...
// api
import { I${tableNameCamel}, ${tableNameCamelCase}Api } from "@/api/${databaseName}/${tableNameCamel}.ts";
import {iconUtil, antdUtil} from "@/utils/projectUtil";

// 需要重复用的API
const a = {
  queryPage: (page) => {
    ${tableNameCamelCase}Api.queryPage(page, table.show).then(({data}) => {
...
const funs = ref([
  ...
  },
  {
    name: "批量删除", icon: "delete-outlined", onClick: () => {
      ...
      ${tableNameCamelCase}Api.removeList(ids).then(() => {
...
const table = reactive({
  ...
  columns: [
      ...
      <#if test.type != "Boolean">
      {
          title: '${test.remarks}', dataIndex: '${test.nameCamelCase}', key: 'id',
          customFilterDropdown: true,
          onFilter: (value, record) =>
              record.${test.nameCamel}.toString().toLowerCase().includes(value.toLowerCase()),
  ...
  // 删除
  onDelete: (id) => {
      ${tableNameCamelCase}Api.remove(id).then(() => {
          message.success("删除成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("删除失败")
  ...
  // 更新
  onUpdate: (item) => {
      ${tableNameCamelCase}Api.update(item).then(() => {
  ...
  save: (key: string) => {
      ...
      ${tableNameCamelCase}Api.update(arr[0]).then(() => {
          message.success("修改成功")
          a.queryPage(table.pagination.current)
      }).catch(() => {
          message.error("修改失败")
...
// 表单
const form = reactive<I${tableNameCamel}>({
  <#list fields as test>
...
const modal = reactive({
  ...
  handleOk: (e: MouseEvent) => {
    modal.visible = false
    ${tableNameCamelCase}Api.add(form).then(()=> {
